  ...
  <div>
    <v-container>
      <v-row>
        ...
        <v-col sm='4' md='2' lg="2" align="end">
          <h1 class="white--text pr-5 pt-4 body-1" id="all" @click="click()">
            See All
          </h1>
        </v-col>
      </v-row>
      <v-row>
        <v-col sm='6' md='4' lg="2" v-for="i in 6"  :key ="i">
          ...
          <songCard v-if="played[i-1].type !== 'artist'"
            :id="played[i-1].id"
            :name="played[i-1].name"
            :images="played[i-1].images"
            :description="played[i-1].description"
            :type="played[i-1].type"
            :artistName="played[i-1].artistName"
          >
          </songCard>
        </v-col>
      </v-row>
...
export default {
  ...
  methods: {
    click() {
      this.$router.push('/home/library/playlists');
    },
  },
};
</script>
